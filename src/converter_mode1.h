	...
	static CTypeIndirectionPrefixAndSuffixItem type_indirection_prefix_and_suffix_modifier_and_code_generator(CIndirectionStateStack& indirection_state_stack,
		...
		if (true) {
			...
			for (size_t j = 0; j < indirection_state_stack.size(); j += 1)
			{
				...
				if (indirection_state_ref.is_known_to_be_used_as_array_iterator() || is_char_star) {
...
#define REGOBJ_TEST1_FLAG false
	...
	inline std::string generate_qtype_replacement_code(clang::QualType qtype, Rewriter &Rewrite, CTUState* state1_ptr = nullptr, EIsFunctionParam is_a_function_parameter = EIsFunctionParam::No, std::optional<clang::StorageDuration> maybe_storage_duration = {}, std::optional<const clang::Decl*> maybe_containing_D = {}, std::optional<clang::TypeLoc> maybe_typeLoc = {}) {
		...
		if (res1.m_seems_to_involve_a_template_param_originally) {
			...
			type. */
			if ("" != res1.m_tparam_that_encompasses_the_direct_type_str) {
				...
				direct_type_str = res1.m_tparam_that_encompasses_the_direct_type_str;
			} else {
	...
	void note_array_determination(Rewriter &Rewrite, CTUState& state1, const CDDeclIndirection& ddecl_indirection) {
		...
		if (ddcs_ref.m_indirection_state_stack.size() >= ddecl_indirection.m_indirection_level) {
			...
			if (!(indirection_state1.is_known_to_be_used_as_array_iterator())) {
				indirection_state1.set_is_known_to_be_used_as_array_iterator(true);
	...
	CArrayInferenceInfo infer_array_type_info_from_stmt_indirection_stack(CDDeclConversionState& ddcs_ref,
		...
		for (size_t i = 0; ((i < ddcs_ref.m_indirection_state_stack.size())
			...
			} else if (!ddcs_indirection_state.is_known_to_be_used_as_array_iterator()) {
				if (("ArraySubscriptExpr" == stmt_indirection_stack[i])
						|| ("pointer arithmetic" == stmt_indirection_stack[i])) {
					ddcs_indirection_state.set_is_known_to_be_used_as_array_iterator(true);
	...
	* the function will execute any queued up actions that were contingent on such a determination. */
	...
	void CAssignmentTargetConstrainsSourceReplacementAction::do_replacement(CTUState& state1) const {
		...
		if (src_ddcs_ref.m_indirection_state_stack.size() > src_indirection_level) {
			...
			if (trgt_indirection_state.is_known_to_be_used_as_array_iterator() && (!src_indirection_state.is_known_to_be_used_as_array_iterator())) {
				src_indirection_state.set_is_known_to_be_used_as_array_iterator(true);
			...
			if (true && (trgt_indirection_state.is_known_to_have_malloc_target() && (!trgt_indirection_state.is_known_to_have_non_malloc_target())) 
				...
				src_update_declaration_flag |= true;
				state1.m_conversion_state_change_action_map.execute_matching_actions(state1, src_indirection);
				if (src_indirection_state.is_known_to_be_used_as_array_iterator()) {
					state1.m_dynamic_array2_contingent_replacement_map.do_and_dispose_matching_replacements(state1, src_indirection);
				}
			}
		} else {
	...
	void CAssignmentSourceConstrainsTargetReplacementAction::do_replacement(CTUState& state1) const {
		...
		if (trgt_ddcs_ref.m_indirection_state_stack.size() > trgt_indirection_level) {
			...
			if (src_indirection_state.is_known_to_be_used_as_array_iterator() && (!trgt_indirection_state.is_known_to_be_used_as_array_iterator())) {
				trgt_indirection_state.set_is_known_to_be_used_as_array_iterator(true);
	...
	void CSameTypeReplacementAction::do_replacement(CTUState& state1) const {
		...
		if ((lhs_ddcs_ref.m_indirection_state_stack.size() > lhs_indirection.m_indirection_level) &&
			...
			if (false) {
				...
				if (rhs_indirection_state_ref.is_known_to_be_used_as_array_iterator() && (!lhs_indirection_state_ref.is_known_to_be_used_as_array_iterator())) {
			...
			if ((COSR.isValid()) && (cond_SR.isValid()) && (lhs_SR.isValid()) && (rhs_SR.isValid())) {
				...
				if ((lhs_is_known_to_be_an_array || rhs_is_known_to_be_an_array) && LHS->getType()->isPointerType()) {
					...
					if (lhs_needs_to_be_wrapped || rhs_needs_to_be_wrapped) {
						...
						if (ddcs_ptr) {
							...
							if ((inference_info.indirection_level + 1) <= ddcs_ref.m_indirection_state_stack.size()) {
								...
								auto res3 = generate_type_indirection_prefix_and_suffix(indirection_state_stack_of_pointee, (*this).m_Rewrite, EIsFunctionParam::No, {}/*maybe_storage_duration*/, &state1);
					...
					if (LHS->getType()->isPointerType()) {
						...
						if (ddcs_ptr) {
							auto& ddcs_ref = *ddcs_ptr;
							auto res3 = generate_type_indirection_prefix_and_suffix(ddcs_ref.m_indirection_state_stack, (*this).m_Rewrite, EIsFunctionParam::No, {}/*maybe_storage_duration*/, &state1);
							auto direct_qtype_str = ddcs_ref.current_direct_qtype_str();
				...
				} else if ((lhs_maybe_function_state.has_value() && lhs_maybe_function_state.value().has_been_changed())
					|| (rhs_maybe_function_state.has_value() && rhs_maybe_function_state.value().has_been_changed())) {
					...
					if (lhs_DD && rhs_DD) {
						if (state1.m_ast_context_ptr) {
							...
							auto lhs_is_non_modifiable = is_non_modifiable(*lhs_DD, *(state1.m_ast_context_ptr), Rewrite, state1);
							auto rhs_is_non_modifiable = is_non_modifiable(*rhs_DD, *(state1.m_ast_context_ptr), Rewrite, state1);
							if (rhs_is_non_modifiable && (!lhs_is_non_modifiable)) {
								maybe_lone_modifiable_arg_info = { lhs_DD, lhs_inference_info.indirection_level, LHS };
							} else if (lhs_is_non_modifiable && (!rhs_is_non_modifiable)) {
								maybe_lone_modifiable_arg_info = { rhs_DD, rhs_inference_info.indirection_level, RHS };
							}
							if (maybe_lone_modifiable_arg_info.has_value()) {
								...
								(function) argument. */
								auto [ddcs_ref, update_declaration_flag] = state1.get_ddecl_conversion_state_ref_and_update_flag(*(lone_modifiable_arg_info.DD), &m_Rewrite);
								...
								auto res3 = generate_type_indirection_prefix_and_suffix(indirection_state_stack_of_expression, (*this).m_Rewrite, EIsFunctionParam::No, {}/*maybe_storage_duration*/, &state1);
								auto direct_qtype_str = ddcs_ref.current_direct_qtype_str();
								...
								auto adj_arg_qtype_str = res3.m_prefix_str + direct_qtype_str + res3.m_suffix_str;
								...
								if ("Dual" == ConvertMode) {
									...
									arg_prefix_str = "(" + function_pointer_type_str + ")(MSE_LH_UNSAFE_MAKE_FN_WRAPPER(";
								} else {
									function_pointer_type_str = std::string("mse::lh::TNativeFunctionPointerReplacement<") + direct_qtype_str + ">";
									arg_prefix_str = function_pointer_type_str + "(mse::us::lh::unsafe_make_fn_wrapper(";
								}

								std::string arg_suffix_str;
								if ("Dual" == ConvertMode) {
									arg_suffix_str = std::string(", ") + function_pointer_type_str + "()))";
								} else {
									arg_suffix_str = std::string(", ") + function_pointer_type_str + "()))";
								}

								cocs_ref.m_arg_prefix_str = arg_prefix_str;
								cocs_ref.m_arg_suffix_str = arg_suffix_str;
								cocs_ref.m_lhs_needs_to_be_cast = true;
								cocs_ref.m_rhs_needs_to_be_cast = true;
							}
						}
			...
			, const clang::ConditionalOperator* CO, std::optional<const DeclaratorDecl*> maybe_DD) 
		{
			...
			if (/*(DS != nullptr) && */(LHS != nullptr) && (RHS != nullptr))
			{
				...
				if (maybe_DD.has_value() && LHS->getType()->isPointerType() && RHS->getType()->isPointerType()) {
					...
					auto cr_shptr = std::make_shared<CConditionalOperatorReconciliation2ReplacementAction>(Rewrite, MR, CDDeclIndirection(*DD, 0), CO, lhs_res2.ddecl_cptr, rhs_res2.ddecl_cptr, possibly_null_DD);
					(*cr_shptr).do_replacement(state1);
					state1.m_conversion_state_change_action_map.insert(cr_shptr);
				}
				...
				if (ConvertToSCPP) {
					...
					* values of the conditional operator must be the same type. */
					if (lhs_res2.ddecl_cptr) {
						...
						auto cr_shptr = std::make_shared<CConditionalOperatorReconciliation2ReplacementAction>(Rewrite, MR, CDDeclIndirection(*lhs_res2.ddecl_cptr, lhs_res2.indirection_level), CO, lhs_res2.ddecl_cptr, rhs_res2.ddecl_cptr, possibly_null_DD);
					...
					if (rhs_res2.ddecl_cptr) {
						...
						auto cr_shptr = std::make_shared<CConditionalOperatorReconciliation2ReplacementAction>(Rewrite, MR, CDDeclIndirection(*rhs_res2.ddecl_cptr, rhs_res2.indirection_level), CO, lhs_res2.ddecl_cptr, rhs_res2.ddecl_cptr, possibly_null_DD);
									...
									if (true || (("void *" != arg_EX_ii_qtype_str) && ("const void *" != arg_EX_ii_qtype_str))) {
										...
										if (is_pointer_to_pointer) {
											if ((!string_begins_with(function_qname, "mse::")) && (!string_begins_with(arg_function_qname_if_any, "mse::"))) {
												...
												if (1 <= arg_EX_ii_ecs_ref.m_expr_text_modifier_stack.size()) {
													if ("unsafe make temporary array of raw pointers from" == arg_EX_ii_ecs_ref.m_expr_text_modifier_stack.back()->species_str()) {
														/* already applied */
														shptr1 = nullptr;
													}
												}
											...
											if ((!string_begins_with(function_qname, "mse::")) && (!string_begins_with(arg_function_qname_if_any, "mse::"))) {
												shptr1 = std::make_shared<CUnsafeMakeRawPointerFromExprTextModifier>();
												if (1 <= arg_EX_ii_ecs_ref.m_expr_text_modifier_stack.size()) {
													if ("unsafe make raw pointer from" == arg_EX_ii_ecs_ref.m_expr_text_modifier_stack.back()->species_str()) {
														/* already applied */
														shptr1 = nullptr;
													}
												}
										...
										if (("void *" != param_VD_qtype_str) && ("const void *" != param_VD_qtype_str)) {
											...
											if (1 <= arg_EX_ii_ecs_ref.m_expr_text_modifier_stack.size()) {
												if ("unsafe cast" == arg_EX_ii_ecs_ref.m_expr_text_modifier_stack.back()->species_str()) {
													/* already applied */
													shptr1 = nullptr;
													//return;
												}
									...
									if (arg_EX->getType()->isPointerType()) {
										...
										if (ConvertToSCPP) {
											...
											if (("void *" != arg_EX_qtype_str) && ("const void *" != arg_EX_qtype_str)) {
												if ((!string_begins_with(function_qname, "mse::")) && (!string_begins_with(arg_function_qname_if_any, "mse::"))) {
													shptr1 = std::make_shared<CUnsafeMakeRawPointerFromExprTextModifier>();
													if (1 <= arg_ecs_ref.m_expr_text_modifier_stack.size()) {
														if ("unsafe make raw pointer from" == arg_ecs_ref.m_expr_text_modifier_stack.back()->species_str()) {
															/* already applied */
															shptr1 = nullptr;
															//return;
														}
