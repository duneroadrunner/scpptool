...
class COrderedRegionSet : public std::set<COrderedSourceRange> {
	...
	public:
		...
		value_ptr1_t(const value_ptr1_t& src) : base_class(std::make_unique<_Ty>(*src)) { }
		value_ptr1_t(value_ptr1_t&& src) : base_class(std::forward<base_class>(src)) { }
	...
	struct CAbstractLifetimeSet {
		...
		bool operator==(const CAbstractLifetimeSet& rhs) const {
			return (rhs.m_primary_lifetimes == m_primary_lifetimes)
				&& (rhs.m_sublifetime_vlptrs == m_sublifetime_vlptrs)
				&& (rhs.m_template_arg_sublifetime_vlptrs == m_template_arg_sublifetime_vlptrs);
		}
		bool operator!=(const CAbstractLifetimeSet& rhs) const {
			return !((*this) == rhs);
		}
		...
		std::vector<value_ptr1_t<CAbstractLifetimeSet> > m_template_arg_sublifetime_vlptrs;
	};
