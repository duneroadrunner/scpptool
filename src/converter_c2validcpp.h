...
namespace convc2validcpp {
	...
	* indirection", respectively, relative to var1. */
	...
	class CSourceRangeContext1 {
		public:
		struct may_be_a_gnu_attr_tag {};
		std::variant<may_be_a_gnu_attr_tag, clang::Expr const*, clang::Decl const*> node_variant;
	};
	...
	static clang::SourceRange cm1_nice_source_range(const clang::SourceRange& sr, clang::Rewriter &Rewrite, CTUState* state1_ptr = nullptr) {
		...
		if ((!(FLSLE < FLSL)) && (retSR.isValid()) && same_file6) {
			std::string text6 = getRewrittenTextOrEmpty(Rewrite, {FLSL, FLSLE});
			int q = 5;
		} else {
			int q = 5;
		}
	...
	static clang::SourceRange cm1_adj_nice_source_range(const clang::SourceRange& sr, CTUState& state1, clang::Rewriter &Rewrite, std::optional<CSourceRangeContext1> maybe_context = {}) {
		const auto adjusted_SR = cm1_adjusted_source_range(sr, state1, Rewrite, maybe_context);
		if (!(adjusted_SR == sr)) {
			return adjusted_SR;
		}
	...
	static clang::SourceRange cm1_adj_nice_source_range(clang::Expr const& expr, CTUState& state1, clang::Rewriter &Rewrite) {
		return cm1_adj_nice_source_range(expr.getSourceRange(), state1, Rewrite, CSourceRangeContext1{ &expr });
	}
	static clang::SourceRange cm1_adj_nice_source_range(clang::Decl const& decl, CTUState& state1, clang::Rewriter &Rewrite) {
		return cm1_adj_nice_source_range(decl.getSourceRange(), state1, Rewrite, CSourceRangeContext1{ &decl });
	}
	...
	clang::QualType populateQTypeIndirectionStack(CIndirectionStateStack& stack, clang::QualType qtype, std::optional<clang::TypeLoc> maybe_typeLoc = {}, Rewriter* Rewrite_ptr = nullptr, CTUState* state1_ptr = nullptr, int depth = 0) {
		...
		if (Rewrite_ptr && l_maybe_typeLoc.has_value()) {
			...
			if ((l_SR).isValid() && (((l_SR).getBegin() < (l_SR).getEnd()) || ((l_SR).getBegin() == (l_SR).getEnd()))) {
				...
				original_source_text = getRewrittenTextOrEmpty(Rewrite, cq_SR);
		...
		if (TP->isArrayType()) {
			...
			if (llvm::isa<const clang::VariableArrayType>(TP)) {
				...
				if (!VATP) {
					...
					//size_text = getRewrittenTextOrEmpty(Rewrite, SR);
				}
		...
		if ((!processed) && maybe_typeLoc.has_value()) {
			...
			if (Rewrite_ptr) {
				...
				if ((l_SR).isValid() && (((l_SR).getBegin() < (l_SR).getEnd()) || ((l_SR).getBegin() == (l_SR).getEnd()))) {
					DEBUG_SOURCE_LOCATION_STR(l_debug_source_location_str, l_SR, Rewrite);
					auto source_text2 = getRewrittenTextOrEmpty(Rewrite, l_SR);
	...
	* (if any) (in the expression) is a pointer dereference or an array subscript. 
				...
				if (1 > individual_declarator_decls.size()) {
					...
					if (int(individual_declarator_decls.size()) <= DD_index) {
						...
						if ((l_SL1 < l_SL2) || (l_SL1 == l_SL2)) {
							...
							//std::string interstitial_text = getRewrittenTextOrEmpty(Rewrite, {l_SL1, l_SL2});
							int q = 5;
						} else {
							assert(false);
						}
				...
				auto maybe_default_context = [&]() -> std::optional<CExprTextInfoContext> {
					if (m_ddecl_cptr) {
						...
						if (state1_ptr && raw_SR.getBegin().isMacroID() && raw_SR.getEnd().isMacroID()) {
							...
							if (SR_plus.isValid()) {
								return CExprTextInfoContext{ SR_plus, &Rewrite, state1_ptr };
							}
						}
					}
					return {};
	...
	public:
		CRecordDeclConversionState(const clang::RecordDecl& recdecl, Rewriter &Rewrite, CTUState& state1) : m_recdecl_ptr(&recdecl) {
			m_original_source_text_str = getRewrittenTextOrEmpty(Rewrite, cm1_adj_nice_source_range(source_range(), state1, Rewrite));
	...
	struct do_not_set_up_child_dependencies_t {};
		CExprConversionState(do_not_set_up_child_dependencies_t, const clang::Expr& expr, Rewriter &Rewrite, CTUState& state1) : m_expr_cptr(&expr), Rewrite(Rewrite), m_state1(state1) {
#ifndef NDEBUG
			auto SR = cm1_adj_nice_source_range(expr, state1, Rewrite);
			DEBUG_SOURCE_LOCATION_STR(debug_source_location_str, SR, Rewrite);
			if (std::string::npos != debug_source_location_str.find(g_target_debug_source_location_str1)) {
				int q = 5;
			}
			...
			m_SR_plus = cm1_adjusted_source_range(expr, state1, Rewrite);
			...
			if (expr_SR.isValid()) {
				...
				if ("" == m_original_source_text_str) {
					m_original_source_text_str = getRewrittenTextOrEmpty(Rewrite, expr_SR);
					if ("" == m_original_source_text_str) {
						auto nice_SR = cm1_adj_nice_source_range(expr, state1, Rewrite);
						if (nice_SR.isValid()) {
							m_original_source_text_str = getRewrittenTextOrEmpty(Rewrite, nice_SR);
						}
					}
				}
			}
		...
		bool set_up_child_dependencies_generically();
		...
		auto maybe_default_context() const -> std::optional<CExprTextInfoContext> {
			if (m_expr_cptr) {
				...
				if (raw_SR.getBegin().isMacroID() && raw_SR.getEnd().isMacroID()) {
					return CExprTextInfoContext{ m_SR_plus, &Rewrite, &m_state1 };
				}
			}
			return {};
		}
	...
	* values of a conditional operator must be the same type. */
	...
	class CCodeModificationActions : public std::map<COrderedSourceRange, std::list<CCodeModificationActionAndID> > {
		...
		std::pair<base_class::iterator, bool> add_replacement_of_instances_of_given_string_action(Rewriter &Rewrite, const COrderedSourceRange& OSR, const std::string& old_string, const std::string& new_string) {
			auto lambda = [this, &Rewrite, OSR, old_string, new_string]() {
					if (OSR.isValid()) {
						std::string l_source_text1 = getRewrittenTextOrEmpty(Rewrite, OSR);
		...
		std::pair<base_class::iterator, bool> add_replacement_of_whole_instances_of_given_string_action(Rewriter &Rewrite, const COrderedSourceRange& OSR, const std::string& old_string, const std::string& new_string) {
			auto lambda = [this, &Rewrite, OSR, old_string, new_string]() {
					if (OSR.isValid()) {
						std::string replacement_code = getRewrittenTextOrEmpty(Rewrite, OSR);
	...
	private:
		...
		expression. If no such object exists, one will be created and stored. If specified template 
		parameter, TExprConversionState, is not (the default) CExprConversionState, and the existing 
		object is not an instance of TExprConversionState, it will replaced by a new object of 
		(CExprConversionState subclass) type TExprConversionState. If the `overwrite` argument value 
		...
		TExprConversionState& get_expr_conversion_state_ref_helper1(bool overwrite, TExpr const& expr, Rewriter &Rewrite, TArgs&&... args) {
...
#ifndef NDEBUG
			auto SR = cm1_adj_nice_source_range(*EX, state1, Rewrite);
			if (SR.isValid()) {
				DEBUG_SOURCE_LOCATION_STR(debug_source_location_str, SR, Rewrite);
				if (std::string::npos != debug_source_location_str.find(g_target_debug_source_location_str1)) {
					int q = 5;
			...
			if (insert_or_assign) {
				auto shptr1 = make_expr_conversion_state_shared_ptr<TExprConversionState>(*EX, Rewrite, state1, std::forward<TArgs>(args)...);
			...
			assert((std::is_same<TExprConversionState, CExprConversionState>::value) || TExprConversionState::s_species() == (*((*excs_iter).second)).species());
			return static_cast<TExprConversionState&>(*((*excs_iter).second));
		}
	...
	public:
		...
		parameter, TExprConversionState, is not (the default) CExprConversionState, and the existing 
		object is not an instance of TExprConversionState, it will replaced by a new object of 
		(CExprConversionState subclass) type TExprConversionState. */
		...
		template<typename TExprConversionState = CExprConversionState, typename TExpr, typename... TArgs>
		TExprConversionState& get_expr_conversion_state_ref(TExpr const& expr, Rewriter &Rewrite, TArgs&&... args) {
		...
		template<typename TExprConversionState = CExprConversionState, typename TExpr, typename... TArgs>
		TExprConversionState& set_expr_conversion_state_ref(TExpr const& expr, Rewriter &Rewrite, TArgs&&... args) {
		...
		std::pair<CCodeModificationActions::base_class::iterator, bool> add_pending_expression_update(TExpr const& expr, Rewriter &Rewrite, TArgs&&... args) {
			...
			auto EXSR = write_once_source_range(cm1_adj_nice_source_range(*EX, state1, Rewrite));
			if (EXSR.isValid()) {
	...
	CExprTextInfo::CExprTextInfo(const clang::Expr * expr_ptr, Rewriter &Rewrite, CTUState& state1) : m_expr_cptr(expr_ptr), m_state1(state1) {
		if (expr_ptr) {
			...
			auto& expr_SR = m_SR_plus;
			if (expr_SR.isValid()) {
				auto text5 = getRewrittenTextOrEmpty(Rewrite, expr_SR);
#ifndef NDEBUG
				if (std::string::npos != expr_SR.m_adjusted_source_text_as_if_expanded.find("ossl_check_const_GENERAL_NAME_sk_type(sk)")) {
					int q = 5;
				}
#endif /*!NDEBUG*/
				//m_original_source_text_str = getRewrittenTextOrEmpty(Rewrite, cm1_adj_nice_source_range(expr, state1, Rewrite));
				...
				if (rawSR.isValid()) {
					...
					if (SL.isMacroID() || SLE.isMacroID()) {
						...
						auto text4 = getRewrittenTextOrEmpty(Rewrite, SPSR2);
										...
										if (parent_b13 || parent_b13b) {
											...
											if (parent_expr_SR.isValid() && (false == parent_expr_SR.m_macro_expansion_range_substituted_with_macro_invocation_range)) {
												...
												if (SPSR2.isValid() && SM.isPointWithin(SPSR2.getBegin(), expr_SR.getBegin(), expr_SR.getEnd())
													...
													auto text_sp2 = getRewrittenTextOrEmpty(Rewrite, SPSR2);
			...
			auto SR = cm1_adj_nice_source_range(expr_ptr->getSourceRange(), state1, Rewrite);
			if (SR.isValid()) {
				m_original_source_text_str = getRewrittenTextOrEmpty(Rewrite, SR);
			*/
			} else {
				int q = 3;
			}
	...
	bool CExprConversionState::set_up_child_dependencies_generically() {
		...
		(visible) child expressions. Classes derived from CExprConversionState may have their own specific 
		rendering implementation, but we need to provide a generic default implementation. In this function 
		we undertake to find and note the expression's visible children, their (rendering) location within 
		the expression, and all the "connecting" source text between the visible children. */
		if ((!m_expr_cptr) || ("" == m_original_source_text_str) || (!m_state1.m_ast_context_ptr)) {
			return false;
		}
		IF_DEBUG(std::string qtype_str = m_expr_cptr->getType().getAsString();)
		auto& SM = m_state1.m_ast_context_ptr->getSourceManager();

#ifndef NDEBUG
		auto SR = cm1_adj_nice_source_range((*m_expr_cptr).getSourceRange(), m_state1, Rewrite);
		DEBUG_SOURCE_LOCATION_STR(debug_source_location_str, SR, Rewrite);
		if (std::string::npos != debug_source_location_str.find(g_target_debug_source_location_str1)) {
			int q = 5;
		}
	...
	std::optional<clang::SourceLocation> matching_close_parentheses_if_any(Rewriter& Rewrite, clang::SourceLocation open_paren_SL, std::optional<clang::SourceLocation> maybe_inclusive_bounding_SL = {}) {
		...
		int nested_depth = 0;
		std::string text1 = getRewrittenTextOrEmpty(Rewrite, { SL, SL });
		...
		while (true) {
			...
			if (1 <= nested_depth) {
				if (true) {
					const std::string text_from_the_open_paren = getRewrittenTextOrEmpty(Rewrite, { open_paren_SL, SL });
					for (size_t i = 0; i < text_from_the_open_paren.length(); ++i) {
						if (text_from_the_open_paren.at(i) != char_data[i]) {
			...
			if (2 <= text1.size()) {
				if (('\"' == text1.front()) && ('\"' == text1.back())) {
					offset += text1.length();
				} else {
					size_t i = 1;
					for (; text1.size() > i; i += 1) {
						auto const& ch = text1[i];
						if (('(' == ch) || (')' == ch)) {
							int q = 5;
							break;
						}
			...
			text1 = getRewrittenTextOrEmpty(Rewrite, { SL, SL });
		};
		return retval;
	}
	...
	static clang::SourceRange extended_to_include_all_directly_enclosing_parens(clang::SourceRange const& SR, CTUState& state1, clang::Rewriter &Rewrite) {
		...
		auto extended_to_include_one_level_of_enclosing_parens_if_present = [](clang::SourceRange const& SR, CTUState& state1, clang::Rewriter &Rewrite) {
			...
			std::string text3;
			if (left_SL1.isValid()) {
				text3 = getRewrittenTextOrEmpty(Rewrite, { left_SL1, left_SL1 });
				while ("" == text3) {
	...
	static std::optional<clang::SourceRange> extended_to_include_prefix_if_present(std::string_view prefix, clang::SourceRange const& SR, CTUState& state1, clang::Rewriter &Rewrite) {
		...
		auto location_of_first_preceding_non_whitespace_if_any = [](clang::SourceLocation const& SL, CTUState& state1, clang::Rewriter &Rewrite) -> std::optional<clang::SourceLocation> {
			...
			if (SL2.isValid()) {
				auto text1 = getRewrittenTextOrEmpty(Rewrite, { SL2, SL2 });
				while (!(("" == text1) || ((1 == text1.length()) && (std::isspace(text1.at(0)))))) {
					SL2 = SL2.getLocWithOffset(-1);
					if (SL2.isValid()) {
						text1 = getRewrittenTextOrEmpty(Rewrite, { SL2, SL2 });
						int q = 5;
					} else {
						return {};
					}
		...
		if (maybe_location_of_first_preceding_non_whitespace.has_value()) {
			...
			if (SL1.isValid()) {
				...
				if ((SR2).isValid() && (((SR2).getBegin() < (SR2).getEnd()) || ((SR2).getBegin() == (SR2).getEnd()))) {
					auto SR2_source_text = getRewrittenTextOrEmpty(Rewrite, SR2);
			...
			if (1 <= arg_str.length()) {
				...
				if (arg_SR.isValid()) {
					...
					if (text2.length() > arg_str.length()) {
						...
						if ('\"' == arg_str.at(0)) {
							auto test1_SR = arg_SR;
							test1_SR.setEnd(test1_SR.getBegin());
							std::string text4 = getRewrittenTextOrEmpty(Rewrite, test1_SR);
							if (text4 == arg_str) {
								arg_SR = test1_SR;
								text2 = text4;
								already_fixed = true;
	...
	static CSourceRangePlus cm1_adjusted_source_range(const clang::SourceRange& sr, CTUState& state1, clang::Rewriter &Rewrite, std::optional<CSourceRangeContext1> maybe_context/* = {}*/) {
		...
		if (maybe_context.has_value()) {
			auto& context = maybe_context.value();
			if (std::get_if<CSourceRangeContext1::may_be_a_gnu_attr_tag>(&(context.node_variant))) {
				may_be_a_gnu_attr = true;
			} else  if (std::get_if<clang::Expr const*>(&(context.node_variant))) {
				is_indicated_to_be_an_expression = true;
			}
		}

		/* The may_be_a_gnu_attr value indicates that the given source range may refer to a gnu 
		...
		if (b3 || b4) {
			...
			/* <Quick-'n-dirty linear cache of results.> */

			struct CSourceRangePlusCacheItem : CSourceRangePlus {
				typedef CSourceRangePlus base_class;
				CSourceRangePlusCacheItem(const CSourceRangePlusCacheItem&) = default;
				CSourceRangePlusCacheItem(CSourceRangePlusCacheItem&&) = default;
				CSourceRangePlusCacheItem() = default;
				CSourceRangePlusCacheItem(base_class sr_plus_info, SourceRange SR) 
					: base_class(std::move(sr_plus_info)), m_SR(SR)  {}

				CSourceRangePlusCacheItem& operator=(const CSourceRangePlusCacheItem&) = default;
			...
			/* </Quick-'n-dirty linear cache of results.> - There is also some cache updating code near the end of this code scope. */
			...
			auto macro_spelling_range_extended_to_include_any_arguments = [&SM, &Rewrite, &state1](clang::SourceRange const& macro_SR) {
					...
					if (("" == macro_name) && adjusted_macro_SPSR.isValid() && ((adjusted_macro_SPSR.getBegin() < adjusted_macro_SPSR.getEnd()) || (adjusted_macro_SPSR.getBegin() == adjusted_macro_SPSR.getEnd()))) {
						...
						be the last token. */
						macro_name = getRewrittenTextOrEmpty(Rewrite,  { adjusted_macro_SPSR.getEnd(), adjusted_macro_SPSR.getEnd() } );
						if (")" == macro_name) {
							macro_name = getRewrittenTextOrEmpty(Rewrite,  { adjusted_macro_SPSR.getBegin(), adjusted_macro_SPSR.getEnd() } );
						}
					...
					if (true && ((state1.m_pp_macro_definitions.end() == found_macro_iter))) {
						...
						const std::string text9 = getRewrittenTextOrEmpty(Rewrite, { macro_SPSL, macro_SPSLE });
									...
									if ((exp_SL <= am_SPSL) && (am_SPSL <= exp_SLE) && (exp_SL <= am_SPSLE) && (am_SPSLE <= exp_SLE)) {
										...
										if (last_non_whitespace_SL.isValid()) {
											...
											while (current_SL.isValid() && (current_SL <= expansion_SLE)) {
												const std::string text3 = getRewrittenTextOrEmpty(Rewrite, { current_SL, current_SL });
												if ("(" == text3) {
				...
				if ((SP2SR).isValid() && (((SP2SR).getBegin() < (SP2SR).getEnd()) || ((SP2SR).getBegin() == (SP2SR).getEnd()))) {
					...
					if ("" != SP2SR_source_text) {
						...
						if ((FLSR).isValid() && (((FLSR).getBegin() < (FLSR).getEnd()) || ((FLSR).getBegin() == (FLSR).getEnd()))) {
							FLSR_source_text = getRewrittenTextOrEmpty(Rewrite, FLSR);
							if ("" != FLSR_source_text) {
								SPSR = FLSR;
			...
			if (may_be_a_gnu_attr) {
				...
				if (maybe_SR2.has_value()) {
					...
					auto text2 = getRewrittenTextOrEmpty(Rewrite, SPSR2);
					if ("" != text2) {
						SPSR = SPSR2;
...

			{
...

				{
					...
					auto first_macro_SL = last_macro1_SL;
					auto first_macro_SLE = last_macro1_SLE;
					...
					if (true && SL_isMacroArgExpansion_flag && SLE_isMacroArgExpansion_flag && (SL_macro_arg_expansion_start == SLE_macro_arg_expansion_start)
						...
						do {
#ifndef NDEBUG
							thread_local size_t count8a = 0;
							if (false) {
								count8a += 1;
								if (0 == (count8a % 256)) {
									auto str1 = gtl_time_use_stats_session1.stats_text1();
									int q = 5;
								}
							}
#endif /*!NDEBUG*/
							...
							if (true) {
								...
								/* Ok, we're not sure exactly how source locations work, but our hypothesis is that if our given source 
								range covers only and all of a macro argument, then whatever item occurs "to the left of" (i.e. at a 
								negative offset to) the start of our given range should not be reported as part of the same macro 
								argument. (Note that the item "to the left", in terms of "raw" source location, may not correspond to 
								the item that would be to the left of the spelling location.) Here we check that this is the case. */
								int offset1 = 1;
								auto item_to_the_left_of_first_macro_SL = first_macro_SL.getLocWithOffset(-offset1);
								auto item_to_the_left_of_first_macro_SPSL = SM.getSpellingLoc(item_to_the_left_of_first_macro_SL);
								std::string item_to_the_left_of_first_macro_text = getRewrittenTextOrEmpty(Rewrite, { item_to_the_left_of_first_macro_SPSL, item_to_the_left_of_first_macro_SPSL });
								int depth = 0;
								bool smoke_test_passed_flag = false;
								while (item_to_the_left_of_first_macro_SL.isValid() && ("" == item_to_the_left_of_first_macro_text) && (500/*arbitrary*/ > offset1)) {
									offset1 += 1;
									item_to_the_left_of_first_macro_SL = first_macro_SL.getLocWithOffset(-offset1);
									item_to_the_left_of_first_macro_SPSL = SM.getSpellingLoc(item_to_the_left_of_first_macro_SL);
									item_to_the_left_of_first_macro_text = getRewrittenTextOrEmpty(Rewrite, { item_to_the_left_of_first_macro_SPSL, item_to_the_left_of_first_macro_SPSL });
								}
								auto SL_item_to_the_left_macro_arg_expansion_start = item_to_the_left_of_first_macro_SL;
								bool SL_item_to_the_left_isMacroArgExpansion_flag = SM.isMacroArgExpansion(item_to_the_left_of_first_macro_SL, &SL_item_to_the_left_macro_arg_expansion_start);
								if (SL_item_to_the_left_isMacroArgExpansion_flag && (SL_macro_arg_expansion_start == SL_item_to_the_left_macro_arg_expansion_start)) {
									break;
								}
							}
							if (true) {
								...
								/* Again, we're not sure exactly how source locations work, but it seems that the delimiter to the right of 
								a macro argument gets reported as part of the same macro argument. Our hypothesis is that if our given 
								source range covers only and all of a macro argument, then whatever item occurs "to the right of" (i.e. at a 
								positive offset to) the right delimiter should not be reported as part of the same macro argument. (Note 
								that the item "to the right", in terms of "raw" source location, may not correspond to the item that would 
								be to the right of the spelling location.) Here we check that this is the case. */

								if (first_macro_SLE < first_macro_SL) {
									/* It seems that the begining and end of the macro argument candidate are not in the same macro. This 
									can, for example, occur in code like this:

									#define MACRO2(y) (y + 5)
									#define MACRO1(x) MACRO2(x.a)
									MACRO1(some_obj)

									In this code, the source range of the expression corresponding to the `y` parameter of `MACRO2()` will 
									be reported as beginning at the start of the `some_obj` argument and ending at the end of the `x.a` 
									argument in the definition body of `MACRO1()`. 

									Even though that's not really a valid or useful source range, we still  want to allow this case as the 
									two ranges encompassing the `y` in the body of `MACRO2()` and the `x.a` macro argument in the body of 
									`MACRO1()` are valid (and useful) source location ranges corresponding to the expression represented 
									by `y` (that would be reported in the return value of this function).
									*/
									int q = 5;
								} else {
									int offset1 = text9.length();
									auto item_to_the_right_of_first_macro_SL = first_macro_SL.getLocWithOffset(+offset1);
									auto item_to_the_right_of_first_macro_SPSL = SM.getSpellingLoc(item_to_the_right_of_first_macro_SL);
									std::string item_to_the_right_of_first_macro_text = getRewrittenTextOrEmpty(Rewrite, { item_to_the_right_of_first_macro_SPSL, item_to_the_right_of_first_macro_SPSL });
									bool smoke_test_passed_flag = false;
									bool right_delimiter_encountered = false;
									while (item_to_the_right_of_first_macro_SL.isValid() && (500/*arbitrary*/ > offset1)) {

										auto SL_item_to_the_right_macro_arg_expansion_start = item_to_the_right_of_first_macro_SL;
										bool SL_item_to_the_right_isMacroArgExpansion_flag = SM.isMacroArgExpansion(item_to_the_right_of_first_macro_SL, &SL_item_to_the_right_macro_arg_expansion_start);
										if (!(SL_item_to_the_right_isMacroArgExpansion_flag && (SL_macro_arg_expansion_start == SL_item_to_the_right_macro_arg_expansion_start))) {
											smoke_test_passed_flag = true;
											break;
										}
										if (right_delimiter_encountered && ("" != item_to_the_right_of_first_macro_text)) {
											//smoke_test_passed_flag = false;
											break;
										}
										if ("" != item_to_the_right_of_first_macro_text) {
											if (text8 == item_to_the_right_of_first_macro_text) {
												right_delimiter_encountered = true;
											} else {
												smoke_test_passed_flag = true;
												break;
											}
										...
										offset1 += 1;
										item_to_the_right_of_first_macro_SL = first_macro_SL.getLocWithOffset(+offset1);
										item_to_the_right_of_first_macro_SPSL = SM.getSpellingLoc(item_to_the_right_of_first_macro_SL);
										item_to_the_right_of_first_macro_text = getRewrittenTextOrEmpty(Rewrite, { item_to_the_right_of_first_macro_SPSL, item_to_the_right_of_first_macro_SPSL });

										thread_local size_t count10b = 0;
										count10b += 1;
									}
									if (!smoke_test_passed_flag) {
										int q = 5;
										break;
									}
								}
...
#ifndef NDEBUG
								if ("beg" == macro_arg_expansion_text1) {
									int q = 5;
								}
#endif /*!NDEBUG*/
				...
				DEBUG_SOURCE_TEXT_STR(last_macro1_source_text2, (clang::SourceRange{ last_macro1_SL, last_macro1_SLE }), Rewrite);
				DEBUG_SOURCE_TEXT_STR(last_macro1_sp_source_text2, (clang::SourceRange{ SM.getSpellingLoc(last_macro1_SL), SM.getSpellingLoc(last_macro1_SLE) }), Rewrite);
						...
						if ((state1.m_pp_macro_definitions.end() != found_macro_iter)) {
							...
							if (presumed_macro_argument_flag) {
								...
								if (macro_params.size() <= macro_args.size()) {
									...
									bool is_known_to_be_variadic = false;
									if (found_macro_iter->second.m_MD_ptr && found_macro_iter->second.m_MD_ptr->getDefinition() && found_macro_iter->second.m_MD_ptr->getDefinition().getMacroInfo()) {
										is_known_to_be_variadic = found_macro_iter->second.m_MD_ptr->getDefinition().getMacroInfo()->isVariadic();
									}
									if (!is_known_to_be_variadic) {
										...
										for (auto& macro_param : macro_params) {
											if (trimmed_source_text_as_if_expanded == macro_param) {
												...
												deeply nested range, it probably wouldn't be valid to use it for anything. So we'll just set it to the 
								...
								adjusted_source_text_info_ref.m_text = retval.m_adjusted_source_text_as_if_expanded;
								adjusted_source_text_info_ref.m_macro_args = macro_args;
								/* If the (entirety of) the macro doesn't seem to represent the same expanded code as the more deeply 
								nested range (i.e. if it represents a superset), then it probably wouldn't be valid to use its range 
								...
								if (is_essentially_the_whole_macro) {
									...
									if (1 <= macro_args.size() || (found_macro_iter->second.m_MD_ptr && found_macro_iter->second.m_MD_ptr->getMacroInfo() 
										...
										for (auto const& macro_arg : macro_args) {
											...
											constructed_invocation_text += macro_arg;
										}
										constructed_invocation_text += ")";
									}
									adjusted_source_text_info_ref.m_text = constructed_invocation_text;
									retval.m_adjusted_source_text_as_if_expanded = constructed_invocation_text;
								}
							}
				...
				if ((true || presumed_macro_argument_flag) && (1 <= retval.m_adjusted_source_text_infos.size()) && ("" != SPSR_source_text)) {
					/* While processing the nested macro invocations we happened to reconstruct, from the most deeply 
					nested macro invocation source range, what should be the text of the given source range (which is 
					presumably a less-deeply nested (or most often, not nested,) macro invocation). To verify the 
					nested macro invocation information that we generated, we'll now check that the reconstructed text 
					(effectively) matches the text of the originally given source range. 

					The texts may not match if, for example, the given source range covered only part of a macro 
					argument. Or if the macro argument does not expand to a whole token. */

					bool reconstructed_macro_invocation_matches_the_original = false;
					for (auto& adjusted_source_text_info : retval.m_adjusted_source_text_infos) {
						if (SPSR_source_text == adjusted_source_text_info.m_text) {
							reconstructed_macro_invocation_matches_the_original = true;
							break;
						} else {
							auto trimmed_SPSR_source_text = SPSR_source_text;
							lrtrim(trimmed_SPSR_source_text);
							auto trimmed_last_text_info_text = adjusted_source_text_info.m_text;
							lrtrim(trimmed_last_text_info_text);
							if (trimmed_SPSR_source_text == trimmed_last_text_info_text) {
								reconstructed_macro_invocation_matches_the_original = true;
								break;
							} else {
#ifndef NDEBUG
								if (trimmed_last_text_info_text.length() > Parse::find_uncommented_token(trimmed_SPSR_source_text, trimmed_last_text_info_text).begin) {
					...
					bool need_confirmation_that_deepest_range_represents_the_expression = false;
					if (!reconstructed_macro_invocation_matches_the_original) {
						need_confirmation_that_deepest_range_represents_the_expression = true;
						auto& adjusted_source_text_info = retval.m_adjusted_source_text_infos.back();
						if (adjusted_source_text_info.m_macro_invocation_range.isValid()) {
							int q = 5;
						}
						/* The outer-most macro invocation source range presumably doesn't correspond with (some or all of) 
						the more deeply nested invocation source ranges, but we may not have yet had the chance to set its 
						reported source range to the default (invalid) value to indicate this fact. */
						adjusted_source_text_info.m_macro_invocation_range = clang::SourceRange{};
					}
					if (1 <= retval.m_adjusted_source_text_infos.size() && retval.m_adjusted_source_text_infos.at(0).m_macro_invocation_range.isValid()) {
						do {
							if (!(maybe_context.has_value())) {
								break;
							}
							...
							if (!(parent_E_rawSR.getBegin().isMacroID() && parent_E_rawSR.getEnd().isMacroID())) {
								if (need_confirmation_that_deepest_range_represents_the_expression) {
									if (E_SR_plus_ref.m_adjusted_source_text_infos.at(0).m_macro_invocation_range != parent_E_rawSR) {
										/* If the expression's (most deeply nested) range and it's parent's (most deeply nested) range are not 
										the same, then we'll presume that the expression's (most deeply nested) range actually represents 
										(just) the expression. (Not a superset of the expression.) */
										need_confirmation_that_deepest_range_represents_the_expression = false;
									}
								}
								break;
							}
							...
							if (!(1 <= parent_E_SR_plus_ref.m_adjusted_source_text_infos.size())) {
								if (need_confirmation_that_deepest_range_represents_the_expression) {
									if ((E_SR_plus_ref.m_adjusted_source_text_infos.at(0).m_macro_invocation_range != parent_E_SR_plus_ref) 
										&& first_has_a_range_contained_in_second(E_SR_plus_ref, parent_E_SR_plus_ref)) {

										/* If the expression's (most deeply nested) range and it's parent's (most deeply nested) range are not 
										the same, then we'll presume that the expression's (most deeply nested) range actually represents 
										(just) the expression. (Not a superset of the expression.) Unless none of the expression's nested 
										(invocation) ranges are contained within the parent's range. A phenomena that could be the result of 
										the parent's nested (invocation) ranges not being reported due to not being supported. In which case 
										we wouldn't know if the expression's (most deeply nested) range is actually different from it's 
										parent's (unreported) most deeply nested range. */
										need_confirmation_that_deepest_range_represents_the_expression = false;
									}
								}
								break;
							...
							if (!E_SR_plus_ref_has_range_contained_in_parent_expression_range) {
								/* None of the reported (nested) macro expansion ranges of the expression seems to be contained inside 
								the range of the parent expression. This has been observed to happen in certain cases where the 
								expression is (the expansion of) a macro argument. At this point there are a couple of possibilities: 
								One is that our previous heuristic attempt to determine whether the originally given source range 
								corresponds to a whole macro argument was inaccurate and it only actually covers part of the macro 
								argument. In this case the "adjusted_source_text_infos" we produced would not be valid and we'd need 
								to remove them.
								The other possibility stems from the fact that the clang library does not seem to always reliably report 
								all of the nested macro invocations where (an expansion of) the macro argument appears. A tentative 
								hypothesis is that the clang library doesn't report macro argument expansions for macros that use 
								arguments more than once in their definition (as the API (we use) doesn't seem to support the reporting 
								of more than one expansion location?). In such cases it would be the most deeply nested macro 
								invocation(s) that would be missing. This means that in such case, the corresponding source range in the 
								most deeply nested macro invocation that does get reported would itself cover a macro argument for an 
								unreported macro invocation. 
								So we can identify cases that fall into this second category by checking if the source range in the most 
								deeply nested macro invocation reported is actually a macro argument (even though it isn't reported as 
								such). We'll try to do that check now: */

								bool seems_to_be_a_macro_argument = false;
								{
									auto const& deepest_reported_adjusted_source_text_info =  E_SR_plus_ref.m_adjusted_source_text_infos.at(0);
									...
									if ((deepest_reported_SL_left_delimiter_is_open_paren_or_comma) && (deepest_reported_SLE_right_delimiter_is_close_paren_or_comma)) {
										std::optional<clang::SourceLocation> maybe_open_paren_SL;
										if (true) {
											/* Here we'll search for the enclosing open parenthesis. */
											int offset1 = 1;
											auto deepest_reported_open_paren_SL = deepest_reported_SL.getLocWithOffset(-offset1);
											auto deepest_reported_open_paren_SPSL = SM.getSpellingLoc(deepest_reported_open_paren_SL);
											std::string deepest_reported_open_paren_text = getRewrittenTextOrEmpty(Rewrite, { deepest_reported_open_paren_SPSL, deepest_reported_open_paren_SPSL });
											int depth = 0;
											while (deepest_reported_open_paren_SL.isValid() && (("(" != deepest_reported_open_paren_text) || (0 != depth)) 
												&& (500/*arbitrary*/ > offset1)) {

												if ("(" == deepest_reported_open_paren_text) {
													depth += 1;
												} else if (")" == deepest_reported_open_paren_text) {
													depth -= 1;
												}
												offset1 += 1;
												deepest_reported_open_paren_SL = deepest_reported_SL.getLocWithOffset(-offset1);
												deepest_reported_open_paren_SPSL = SM.getSpellingLoc(deepest_reported_open_paren_SL);
												deepest_reported_open_paren_text = getRewrittenTextOrEmpty(Rewrite, { deepest_reported_open_paren_SPSL, deepest_reported_open_paren_SPSL });
											}
											if ("(" == deepest_reported_open_paren_text) {
												maybe_open_paren_SL = deepest_reported_open_paren_SL;
											}
										}
										if (maybe_open_paren_SL.has_value()) {
											...
											do {
												...
												auto macro_name_SL = macro_name_SLE;
												auto macro_name_SL_text = getRewrittenTextOrEmpty(Rewrite, { macro_name_SL, macro_name_SL });
												int offset2 = 0;
												auto last_macro_name_SL = macro_name_SL;
												while (macro_name_SL.isValid() && (1 <= macro_name_SL_text.size()) && Parse::is_alnum_or_underscore(macro_name_SL_text.at(0))) {
													last_macro_name_SL = macro_name_SL;
													offset2 += 1;
													macro_name_SL = macro_name_SLE.getLocWithOffset(-offset2);
													macro_name_SL_text = getRewrittenTextOrEmpty(Rewrite, { macro_name_SL, macro_name_SL });
												}
												macro_name_SL = last_macro_name_SL;
												auto macro_name_text = getRewrittenTextOrEmpty(Rewrite, { macro_name_SL, macro_name_SLE });
												auto found_macro_iter = state1.m_pp_macro_definitions.find(macro_name_text);
												if (state1.m_pp_macro_definitions.end() != found_macro_iter) {
													if (1 <= found_macro_iter->second.m_parameter_names.size()) {
														seems_to_be_a_macro_argument = true;
													}
												}
											} while (false);
										}
								...
								if (!seems_to_be_a_macro_argument) {
									/* The expansion range of the most deeply nested macro invocation reported does not seem to be itself 
									a macro argument. So the fact that given source range is not contained within the parent expression's 
									range cannot be explained by unreported (more deeply nested) macro invocations. The remaining 
									explanation would be that the originally given source range does not cover the entire macro argument. 
									In which case, the "adjusted_source_text_infos" we produced which contain the adjusted representation 
									of the range text at the various levels of macro nesting would not be valid. So we'll just bin them. */
									retval.m_adjusted_source_text_as_if_expanded = SPSR_source_text;
									retval.set_source_range(SPSR);
									retval.m_adjusted_source_text_infos.clear();

									if (need_confirmation_that_deepest_range_represents_the_expression) {
										/* The negative condition has just been confirmed and  handled so this other criteria for identifying the 
										negative condition is now presumably redundant. */
										need_confirmation_that_deepest_range_represents_the_expression = false;
									}
								} else {
									/* We are going to assume that the expression text within the parent expression is simply a single 
									(macro parameter) token. The location of that token is situation-specific and depends on the specifics 
							...
							if (need_confirmation_that_deepest_range_represents_the_expression) {
								if (1 <= E_SR_plus_ref.m_adjusted_source_text_infos.size()) {
									if ((E_SR_plus_ref.m_adjusted_source_text_infos.at(0).m_macro_invocation_range != parent_E_SR_plus_ref.m_adjusted_source_text_infos.at(0).m_macro_invocation_range)) {
										/* If the expression's (most deeply nested) range and it's parent's (most deeply nested) range are not 
										the same, then we'll presume that the expression's (most deeply nested) range actually represents 
										(just) the expression. (Not a superset of the expression.) */
										need_confirmation_that_deepest_range_represents_the_expression = false;
					...
					if (need_confirmation_that_deepest_range_represents_the_expression) {
						...
						does not expand to a whole token. In order to rule pout the former, we needed confirmation that 
						the most deeply nested invocation range represents (only) the expression. Apparently we didn't 
						get that confirmation. So presumably it wouldn't be helpful to provide information about the 
						nested invocation of macros that don't actually represent the expression of interest. So we'll 
						just bin them. */
						retval.m_adjusted_source_text_as_if_expanded = SPSR_source_text;
						retval.set_source_range(SPSR);
						retval.m_adjusted_source_text_infos.clear();
	...
	std::vector<clang::SourceRange> enclosing_parentheses(Rewriter& Rewrite, clang::SourceRange enclosed_SR, std::optional<clang::SourceRange> maybe_bounding_SR) {
		...
		text1 = getRewrittenTextOrEmpty(Rewrite, { SLE, SLE });
		while ((")" != text1) || (0 != nested_depth)) {
			if (")" == text1) {
				nested_depth -= 1;
			} else if ("(" == text1) {
	...
	std::optional<CTParamUsageInfo> seems_to_contain_an_instantiation_of_a_template_parameter(const clang::Decl& containing_decl, clang::SourceRange SR, Rewriter &Rewrite, CTUState* state1_ptr = nullptr) {
		...
		if ((SR).isValid() && (((SR).getBegin() < (SR).getEnd()) || ((SR).getBegin() == (SR).getEnd()))) {
			source_text = getRewrittenTextOrEmpty(Rewrite, SR);
		}
	...
	inline std::string current_params_string(Rewriter &Rewrite, CFunctionTypeState& function_type_state_ref, clang::FunctionProtoTypeLoc functionProtoTypeLoc, bool suppress_modifications = false, CTUState *state1_ptr = nullptr, bool is_declaration = false) {
		...
		if (FND && state1_ptr) {
			...
			if (parens_SR.isValid()) {
				...
				std::string parens_text = getRewrittenTextOrEmpty(Rewrite, parens_SR);
				retval = parens_text;
	...
	static CTypeIndirectionPrefixAndSuffixItem type_indirection_prefix_and_suffix_modifier_and_code_generator(CIndirectionStateStack& indirection_state_stack,
		...
		auto additional_void_star_replacement_criteria = [&]() {
			...
			if (indirection_state_stack.m_maybe_DD.has_value()) {
				...
				if (SR.isValid() && ((SR.getBegin() < SR.getEnd()) || (SR.getBegin() == SR.getEnd()))) {
					source_text1 = getRewrittenTextOrEmpty(Rewrite, SR);
				}
				...
				if (indirection_state_ref.m_maybe_typeLoc.has_value() && pointee_maybe_typeLoc.has_value()) {
					...
					if (is_typedef) {
						auto TDDSR = state1_ptr
							? cm1_adj_nice_source_range(definition_TypeLoc(typeLoc).getSourceRange(), *state1_ptr, Rewrite)
							: cm1_nice_source_range(definition_TypeLoc(typeLoc).getSourceRange(), Rewrite);
						std::string TDD_text = getRewrittenTextOrEmpty(Rewrite, TDDSR);
						...
						if (tdtl) {
							...
							if (TDND) {
								if (false) {
									...
									std::string name_text = getRewrittenTextOrEmpty(Rewrite, name_SR1);
									int offset1 = name_text.length() - 1;
									if (0 > offset1) {
										offset1 = 0;
									}
									auto name_SR = clang::SourceRange{ name_SL, name_SL.getLocWithOffset(offset1) };
									name_text = getRewrittenTextOrEmpty(Rewrite, name_SR);
								}
								int q = 5;
							} else {
								int q = 3;
					...
					if (needs_processing && SR.isValid()) {
						std::string old_definition_text = getRewrittenTextOrEmpty(Rewrite, definition_SR);
							...
							if (maybe_name_SL.has_value()) {
								...
								if (!((definition_SR.getEnd() < name_SR.getBegin()) || (name_SR.getEnd() < definition_SR.getBegin()))) {
									...
									if (1 <= enclosing_parentheses1.size()) {
										...
										IF_DEBUG(std::string enclosing_parentheses_text = getRewrittenTextOrEmpty(Rewrite, enclosing_parentheses1.back());)
										...
										if (!(new_SLE < definition_SR.getBegin())) {
											...
											old_definition_text = getRewrittenTextOrEmpty(Rewrite, definition_SR);
											int q = 5;
										} else {
											int q = 3;
										}
											...
											if (ConvertC2ValidCpp && (!suppress_modifications) && state1_ptr) {
												...
												}
												{
													...
													while (!(enclosing_parentheses1.back().getEnd() < SL2)) {
														...
														std::string text1 = getRewrittenTextOrEmpty(Rewrite, SR2);
														for (auto& ch : text1) {
															ch = ' ';
														}
						...
						if (pointee_maybe_stored_source_range.value().isValid()) {
							...
							old_pointee_text = getRewrittenTextOrEmpty(Rewrite, pointee_SR);
							...
							if (pointee_including_any_const_qualifier_needs_processing) {
								...
								old_pointee_text = getRewrittenTextOrEmpty(Rewrite, cq_pointee_SR);
							...
							std::string old_definition_text2 = getRewrittenTextOrEmpty(Rewrite, lexical_SR);
							...
							std::string old_pointee_text2 = getRewrittenTextOrEmpty(Rewrite, lexical_cq_pointee_SR);
							...
							if (lexical_suffix_SR.isValid() && (!(lexical_suffix_SR.getEnd() < lexical_suffix_SR.getBegin()))) {
								old_suffix_text = getRewrittenTextOrEmpty(Rewrite, lexical_suffix_SR);
							...
							if (!indirection_state_ref.m_array_size_expr_read_from_source_text) {
								...
								if (ArrayLoc) {
									...
									if (size_expr_SR.isValid() && (!(size_expr_SR.getEnd() < size_expr_SR.getBegin()))) {
										std::string size_expr_text = getRewrittenTextOrEmpty(Rewrite, size_expr_SR);
										if ("" != size_expr_text) {
...
#define REGOBJ_TEST1_FLAG false
	...
	static CDeclarationReplacementCodeItem declaration_modifier_helper1(const DeclaratorDecl* DD,
		...
		std::string initialization_expr_str{ ddcs_ref.current_initialization_expr_str(Rewrite, state1_ptr, CExprTextInfoContext{ decl_source_range, &Rewrite, state1_ptr }) };
						...
						if (init_expr_source_range.isValid() && init_expr_located_in_this_decl) {
							...
							if (variable_name == initialization_expr_str) {
								...
								* when actually no initialization expression was present in the original source. */
								...
								if (ddcs_ref.current_initialization_expr_str(Rewrite, state1_ptr, CExprTextInfoContext{ decl_source_range, &Rewrite, state1_ptr }).empty()) {
									if (state1_ptr) {
					...
					if (!satisfies_checks) {
						if (clang::StorageDuration::SD_Static == storage_duration) {
							if (!ddcs_ref.m_has_been_replaced_as_a_whole) {
								...
								std::string l_source_text1 = getRewrittenTextOrEmpty(Rewrite, SR);
		...
		if (!(ddcs_ref.m_original_source_text_has_been_noted)) {
			ddcs_ref.m_original_source_text_str = getRewrittenTextOrEmpty(Rewrite, decl_source_range);
			if (FND) {
				...
				if ((REGOBJ_TEST1_FLAG || (ddcs_ref.direct_type_state_ref().is_known_to_be_a_pointer_target()))) {
					...
					if (b1) {
						if ((0 == ddcs_ref.m_indirection_state_stack.size())
							...
							&& (string_begins_with(ddcs_ref.current_initialization_expr_str(Rewrite, state1_ptr, CExprTextInfoContext{ decl_source_range, &Rewrite, state1_ptr }), "{"))) {
		...
		}
		{
			...
			if (0 == ddcs_ref.m_indirection_state_stack.size()) {
				...
				if (ddcs_ref.direct_qtype_has_been_changed()) {
					...
					IF_DEBUG(auto old_text1 = getRewrittenTextOrEmpty(Rewrite, directTypeSR);)
					if (directTypeSR.getBegin().isMacroID() || ddcs_ref.m_ddecl_cptr->getSourceRange().getBegin().isMacroID()) {
						...
						IF_DEBUG(auto old_text1b = getRewrittenTextOrEmpty(Rewrite, directTypeSR);)
						int q = 5;
					}
					auto cq_direct_type_SR = extended_to_include_west_const_if_any(Rewrite, directTypeSR);
					IF_DEBUG(auto old_text2 = getRewrittenTextOrEmpty(Rewrite, cq_direct_type_SR);)
					cq_direct_type_SR = extended_to_include_east_const_if_any(Rewrite, cq_direct_type_SR);
					IF_DEBUG(auto old_text3 = getRewrittenTextOrEmpty(Rewrite, cq_direct_type_SR);)
					...
					if (initializer_SR_ptr) {
						if (true || !(*state1_ptr).m_pending_code_modification_actions.m_already_modified_regions.properly_contains(*initializer_SR_ptr)) {
							if (ddcs_ref.initializer_has_been_changed(Rewrite, state1_ptr)) {
								std::string initializer_str = ddcs_ref.current_initialization_expr_str(Rewrite, state1_ptr, CExprTextInfoContext{ decl_source_range, &Rewrite, state1_ptr });
						...
						if (true || !(*state1_ptr).m_pending_code_modification_actions.m_already_modified_regions.contains({ insert_after_point, *insert_before_point_ptr })) {
							...
							if ("Dual" == ConvertMode) {
								initializer_str = " MSE_LH_IF_ENABLED( = " + ddcs_ref.current_initialization_expr_str(Rewrite, state1_ptr, CExprTextInfoContext{ decl_source_range, &Rewrite, state1_ptr }) + " )";
							} else {
								initializer_str = " = " + ddcs_ref.current_initialization_expr_str(Rewrite, state1_ptr, CExprTextInfoContext{ decl_source_range, &Rewrite, state1_ptr });
							}
	...
	static void declaration_modifier(const DeclaratorDecl& ddecl, Rewriter &Rewrite, CTUState& state1, std::string options_str = "") {
		...
		if (true && (TP->isFunctionType())) {
			...
			if (FND) {
				...
				if (ConvertC2ValidCpp && return_type_source_range.isValid() && (1 <= res.m_replacement_code.size())
					...
					if (return_type_source_range.isValid()/* && res.m_replacement_return_type_post_params_suffix_str.empty()*/) {
						...
						IF_DEBUG(std::string code_to_be_replaced = getRewrittenTextOrEmpty(Rewrite, SR);)
						...
						IF_DEBUG(std::string text1 = getRewrittenTextOrEmpty(Rewrite, SR);)
						auto SR2 = SR;
						auto B = FND->getBody();
						if (B) {
						...
						IF_DEBUG(std::string code_to_be_replaced = getRewrittenTextOrEmpty(Rewrite, SR2);)
			...
			auto check_for_and_handle_unsupported_element2 = [&Rewrite, &unsupported_elements_encounterred, &SR](const clang::TypeLoc& typeLoc, clang::SourceRange l_SR, CTUState& state1) {
				...
				//IF_DEBUG(std::string old_text = getRewrittenTextOrEmpty(Rewrite, type_SR);)
	...
	* the function will execute any queued up actions that were contingent on such a determination. */
	...
	inline CCStyleCastReplacementCodeItem generate_c_style_cast_replacement_code(Rewriter &Rewrite, const clang::CStyleCastExpr* CSCE, std::optional<std::string> maybe_replacement_qtype_str = {}) {
		...
		if (CSCE) {
			auto spelling_source_range = [&Rewrite](const clang::SourceRange& sr) {
				if (true || sr.getBegin().isMacroID()) {
					if (Rewrite.getSourceMgr().isMacroBodyExpansion(sr.getBegin())) {
						...
						if (spelling_SR.isValid()) {
							auto spelling_text = getRewrittenTextOrEmpty(Rewrite, spelling_SR);
				...
				auto source_text = getRewrittenTextOrEmpty(Rewrite, sr);
	...
	void CConditionalOperatorReconciliation2ReplacementAction::do_replacement(CTUState& state1) const {
		...
		if ((COND != nullptr) && (LHS != nullptr) && (RHS != nullptr) && (lhs_DD != nullptr) && (rhs_DD != nullptr)) {
			...
			auto rhs_SR = write_once_source_range(cm1_adj_nice_source_range(*RHS, state1, (*this).m_Rewrite));
	...
	: Rewrite(Rewrite), m_state1(state1) {}
		...
		virtual void run(const MatchFinder::MatchResult &MR)
		{
			...
			if ((E != nullptr))
			{
				...
				auto SR = write_once_source_range(cm1_adj_nice_source_range(*E, m_state1, Rewrite));
				RETURN_IF_SOURCE_RANGE_IS_NOT_VALID1;
					...
					if (subE) {
						...
						if (clang::Stmt::StmtClass::ArraySubscriptExprClass == subE_ii->getStmtClass()) {
							...
							if (adjusted_DRE_ii != subE_ii) {
								...
								DeclRefExpr). */
								std::string adjusted_DRE_ii_source_text =  getRewrittenTextOrEmpty(Rewrite, adjusted_DRE_ii->IgnoreParenImpCasts()->getSourceRange());
								std::string subE_ii_source_text =  getRewrittenTextOrEmpty(Rewrite, subE_ii->getSourceRange());
								return;
							}
						}
					}
									...
									if (true) {
										if (nullptr != DD) {
											...
											if ("" != element_type_str) {
												...
												std::string decl_source_text;
												if (decl_source_range.isValid()) {
													IF_DEBUG(decl_source_text = getRewrittenTextOrEmpty(Rewrite, decl_source_range);)
												} else {
													return;
												}
				...
				if (function_decl && (1 == num_args)) {
					{
						...
						if (ends_with_free) {
							...
							if (arg_source_range.isValid()) {
								arg_source_text = getRewrittenTextOrEmpty(Rewrite, arg_source_range);
								...
								if (arg_QT->isPointerType()) {
									...
									if (callee_raw_SR.getBegin().isMacroID()) {
										...
										std::string callee_spelling_text = getRewrittenTextOrEmpty(Rewrite, callee_spelling_SR);
									...
									if (callee_SR.isValid()) {
										IF_DEBUG(auto callee_text = getRewrittenTextOrEmpty(Rewrite, callee_SR);)
										IF_DEBUG(auto callee_text1 = getRewrittenTextOrEmpty(Rewrite, callee_raw_SR);)
			...
			if ((BO != nullptr) && (RHS != nullptr) && (LHS != nullptr) && (DRE != nullptr))
			{
				...
				if (false && (clang::Expr::NPCK_NotNull != kind)) {
					...
					if (lhs_source_range.isValid()) {
						lhs_source_text = getRewrittenTextOrEmpty(Rewrite, lhs_source_range);
	...
	inline static void handle_c_style_cast_without_context(const MatchFinder::MatchResult &MR, Rewriter &Rewrite, CTUState& state1
		...
		if (CSCE) {
			...
			if ((csce_QT->isPointerType() || csce_QT->isArrayType())
				...
				if (ConvertC2ValidCpp) {
					std::string og_cast_operation_str = getRewrittenTextOrEmpty(Rewrite, cast_operation_SR);
					...
					if ("void *" == csce_QT.getAsString()) {
						...
						if (false) {
							...
							std::string cast_expr_str = getRewrittenTextOrEmpty(Rewrite, precasted_expr_SR);
			...
			if ((csce_QT->isPointerType() || csce_QT->isArrayType())
				...
				if (ConvertC2ValidCpp) {
					...
					if (CXXSCE->getSourceRange().getBegin().isMacroID()) {
						IF_DEBUG(std::string og_whole_expression_str = getRewrittenTextOrEmpty(Rewrite, CXXSCESR);)
						std::string og_precasted_expr_str = getRewrittenTextOrEmpty(Rewrite, precasted_expr_SR);
						...
						IF_DEBUG(std::string og_angle_brackets_str = getRewrittenTextOrEmpty(Rewrite, angle_brackets_SR);)
			...
			if (CE != nullptr)
			{
				...
				if (function_decl1) {
					...
					if (DD && EX) {
						...
						if (DDSR.isValid()) {
							IF_DEBUG(ddecl_source_text = getRewrittenTextOrEmpty(Rewrite, DDSR);)
						} else {
							return;
						}
						...
						if (EXSR.isValid()) {
							IF_DEBUG(expr_source_text = getRewrittenTextOrEmpty(Rewrite, EXSR);)
						} else {
							return;
						}
			...
			if ((CE != nullptr) && (DRE != nullptr))
			{
				...
				if (function_decl && (4 == num_args)) {
					...
					if (fread_str == function_name) {
						...
						auto first_arg_to_end_text = getRewrittenTextOrEmpty(Rewrite, first_arg_to_end_SR);
								...
								for (const auto& macro2_SR : nested_macro_ranges) {
									if (!c2v_filtered_out_by_location(SM, macro2_SR.getBegin())) {
										...
										if ((!(adjusted_macro_SPSR.getBegin() < expansion_SR.getBegin())) && (!(expansion_SR.getEnd() < adjusted_macro_SPSR.getEnd()))
											...
											if ("(" == text1) {
												...
												if (maybe_close_paren_SL.has_value()) {
													...
													macro_args_text = getRewrittenTextOrEmpty(Rewrite, { macro_args_SL, macro_args_SLE });
										...
										if (true) {
											auto found_macro_iter = state1.m_pp_macro_definitions.find(macro_name);
											if (state1.m_pp_macro_definitions.end() != found_macro_iter) {
												macro_decl_text = getRewrittenTextOrEmpty(Rewrite, found_macro_iter->second.definition_SR());
