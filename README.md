...
Unfortunately even these minimized changes are not insignificant. For example, the tool considers the standard library containers "[unsafe](https://godbolt.org/z/aGedWEqYs)" (and will complain when they are used), and instead provides largely compatible replacement implementations. The other big restriction probably being that null values are not supported for raw pointers.
